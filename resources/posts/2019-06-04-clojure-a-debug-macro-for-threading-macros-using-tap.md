...
Rewriting our debug function as a macro is relatively straight forwards we change `defn` to `defmacro`, syntax quote the do` form with <code>`</code> , and unquote the `args` with `~`. Finally, we use `quote` to prevent the `args` from being evaluated.
